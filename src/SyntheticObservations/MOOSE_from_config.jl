...
using ..MOOSE: PARSEC_TO_CM, WolfireConstants, ProcessSynchrotron, format_duration,
                print_logo, print_progress, save_config, write_summary_log

const DEFAULT_BOX_LENGTH_PC = (; x = 50.0, y = 50.0, z = 50.0)
const DEFAULT_BOX_LENGTH_PIX = (; x = 256, y = 256, z = 256)
...
function normalize_box_lengths(box_length)
    ...
    elseif box_length isa AbstractDict
        return (; x = Float64(get(box_length, "x", get(box_length, "X", get(box_length, "size_pc", DEFAULT_BOX_LENGTH_PC.x)))),
                 y = Float64(get(box_length, "y", get(box_length, "Y", get(box_length, "size_pc", DEFAULT_BOX_LENGTH_PC.y)))),
                 z = Float64(get(box_length, "z", get(box_length, "Z", get(box_length, "size_pc", DEFAULT_BOX_LENGTH_PC.z)))))
    else
...
function normalize_box_pixels(box_pixels)
    ...
    elseif box_pixels isa AbstractDict
        return (; x = Int(get(box_pixels, "x", get(box_pixels, "X", get(box_pixels, "npix", DEFAULT_BOX_LENGTH_PIX.x)))),
                 y = Int(get(box_pixels, "y", get(box_pixels, "Y", get(box_pixels, "npix", DEFAULT_BOX_LENGTH_PIX.y)))),
                 z = Int(get(box_pixels, "z", get(box_pixels, "Z", get(box_pixels, "npix", DEFAULT_BOX_LENGTH_PIX.z)))))
    else
